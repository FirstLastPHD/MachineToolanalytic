...
import com.zps.machine.helpers.Complex;
import com.zps.machine.helpers.FFT;
...
public class Spectrum<T extends Number> {
  ...
  /* Return Double Freqw/Power Spectrum */
  public double[][] ReturnSpectrum() {
	   
	  double[] freqw = new double[frequencyBins.length];
	  double[] power  = new double[frequencyBins.length];
	    for (int i = 0; i < frequencyBins.length; i++) {
	    	freqw[i] = frequencyBins[i];
	    	power[i] = spectrum[i];
	    }
	   double[][] returnSpectrum = {freqw,power};
