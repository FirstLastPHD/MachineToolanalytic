 ...
 import javax.sound.sampled.AudioInputStream;
  ...
  public class STFT {
   ...
   // The line should be open, but not started yet.
    public STFT(double[] input, int frameLen, int hopsize, int history) {
    ...
    // The line should be open, but not started yet.
    public STFT(double[] input, int frameLen, int history) {
  ...
  public void removeFrameListener(FrameListener fl) {
       ...
       if (format.isBigEndian()) {
         for (int i = 0; i < audioData.length; i++) {
          ...
          int LSB = (int) audioBytes[2*i+1];
           audioData[i] = ((double)(MSB << 8 | (255 & LSB))) / 32768.0;
         }
       } else {
         for (int i = 0; i < audioData.length; i++) {
   ...
   public static double[] getSTFT(double[] samples, int nfft)
   {
       return STFT.getSTFT(samples, nfft, nfft);
    }
 
   public static double[] getSTFT(double[] samples, int nfft, int nhop)
    {
       double[][] freq = new double[nfft/2+1][samples.length/nhop];
  
        FFT fft = new FFT(nfft);
       double[] window = fft.getWindow();
       double[] mag = new double[freq.length];
       ...
       for(int currFrame = 0; currFrame < samples.length/nhop; currFrame++)
      {
          ...
          int zeroPadLen = currFrame*nhop + wav.length - samples.length;
           ...
           for(int i=0; i<mag.length; i++) {
               mag[i] = 10*Math.log(re[i]*re[i] + im[i]*im[i] + epsilon) / log10;
               System.out.println(" Mag: "+mag[i]);
           }
           System.out.println(" Mag: "+mag.length);   
